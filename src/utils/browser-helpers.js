/**
 * 浏览器端注入辅助脚本
 * 整合了 Chrome 扩展 inject-scripts 的所有功能
 * 适配 Electron 环境
 */
...
function isElementBasicVisible(element) {
  ...
  // 允许 A 标签零尺寸（可能是锚点链接）
  if (rect.width === 0 && rect.height === 0 && element.tagName !== 'A') {
    return false;
  }

  // 宽松检查：只要在 DOM 中且 CSS 属性允许，就认为可见
  // 不检查是否在视口内，也不检查是否被遮挡
  return true;
}

/**
 * 检查元素是否可见（严格检查，包括遮挡检测）
 * @param {HTMLElement} element - 要检查的 DOM 元素
 * @returns {boolean} 如果元素可见且未被遮挡返回 true，否则返回 false
 */
...
/**
 * 获取元素信息
 * @param {HTMLElement} element - 要获取信息的 DOM 元素
 * @returns {Object} 包含元素信息的对象
 * @returns {string} returns.tagName - 元素标签名
 * @returns {string} returns.id - 元素 ID
 * @returns {string} returns.className - 元素类名
 * @returns {string} returns.text - 元素文本内容（前100个字符）
 * @returns {string|null} returns.href - 链接地址（如果是链接元素）
 * @returns {string|null} returns.type - 输入框类型（如果是输入元素）
 * @returns {boolean} returns.isVisible - 元素是否可见
 * @returns {Object} returns.rect - 元素位置和尺寸信息
 * @returns {number} returns.rect.x - 元素 X 坐标
 * @returns {number} returns.rect.y - 元素 Y 坐标
 * @returns {number} returns.rect.width - 元素宽度
 * @returns {number} returns.rect.height - 元素高度
 * @returns {number} returns.rect.top - 元素顶部位置
 * @returns {number} returns.rect.right - 元素右侧位置
 * @returns {number} returns.rect.bottom - 元素底部位置
 * @returns {number} returns.rect.left - 元素左侧位置
 */
...
/**
 * 模拟鼠标点击
 * @param {number} x - 点击位置的 X 坐标（相对于视口）
 * @param {number} y - 点击位置的 Y 坐标（相对于视口）
 * @returns {void}
 */
...
/**
 * 点击元素
 * @param {string} selector - CSS 选择器，用于定位要点击的元素
 * @param {boolean} [waitForNavigation=false] - 是否等待页面导航完成
 * @param {number} [timeout=5000] - 等待导航的超时时间（毫秒）
 * @param {Object|null} [coordinates=null] - 可选的坐标对象，用于指定点击位置
 * @param {number} [coordinates.x] - 点击位置的 X 坐标
 * @param {number} [coordinates.y] - 点击位置的 Y 坐标
 * @returns {Promise<Object>} 返回点击操作的结果对象
 * @returns {boolean} returns.success - 操作是否成功
 * @returns {string} [returns.message] - 操作结果消息
 * @returns {string} [returns.error] - 错误信息（如果操作失败）
 * @returns {Object} [returns.elementInfo] - 被点击元素的信息
 * @returns {boolean} [returns.navigationOccurred] - 是否发生了页面导航
 */
async function clickElement(selector, waitForNavigation = false, timeout = 5000, coordinates = null) {
  try {
    ...
    if (coordinates && typeof coordinates.x === 'number' && typeof coordinates.y === 'number') {
      ...
      elementInfo = { ...getElementInfo(element), clickMethod: 'selector' };
      ...
      if (!elementInfo.isVisible) {
        return { error: `元素不可见: ${selector}`, elementInfo };
      }
    ...
    return { error: `点击元素失败: ${error.message}` };
  }
}
...
/**
 * 填充表单元素
 * @param {string} selector - CSS 选择器，用于定位要填充的表单元素
 * @param {string} value - 要填充的值
 * @returns {Promise<Object>} 返回填充操作的结果对象
 * @returns {boolean} returns.success - 操作是否成功
 * @returns {string} [returns.message] - 操作结果消息
 * @returns {string} [returns.error] - 错误信息（如果操作失败）
 * @returns {Object} [returns.elementInfo] - 表单元素的信息
 * @returns {string} [returns.elementInfo.value] - 填充后的元素值
 */
async function fillElement(selector, value) {
  try {
    ...
    const validInputTypes = [
      ...
      'tel', 'url', 'date', 'datetime-local', 'month', 'time', 'week', 'color',
    ];

    if (!validTags.includes(element.tagName)) {
      const elementInfo = getElementInfo(element);
      return {
        error: `元素不可填充（必须是 INPUT、TEXTAREA 或 SELECT）: ${selector}`,
        elementInfo,
      };
    }
    ...
    if (
      ...
      const elementInfo = getElementInfo(element);
      return {
        error: `输入框类型 "${element.type}" 不可填充: ${selector}`,
        elementInfo,
      };
    }
    ...
    // 使用宽松的可见性检查（只检查 CSS 属性，不检查遮挡）
    const isVisible = isElementBasicVisible(element);
    const elementInfo = getElementInfo(element);
    ...
    return { error: `填充元素失败: ${error.message}` };
  }
}
...
/**
 * 解析按键组合字符串
 * @param {string} keyString - 按键组合字符串，如 "Ctrl+A"、"Enter"、"a" 等
 * @returns {Object|null} 返回解析后的按键信息对象，如果解析失败返回 null
 * @returns {string} returns.key - 按键名称（如 "Enter"、"a"）
 * @returns {string} returns.code - 按键代码（如 "Enter"、"KeyA"）
 * @returns {number} returns.keyCode - 按键码值
 * @returns {number} [returns.charCode] - 字符码值（仅限字符按键）
 * @returns {Object} returns.modifiers - 修饰键状态
 * @returns {boolean} returns.modifiers.ctrlKey - Ctrl 键是否按下
 * @returns {boolean} returns.modifiers.altKey - Alt 键是否按下
 * @returns {boolean} returns.modifiers.shiftKey - Shift 键是否按下
 * @returns {boolean} returns.modifiers.metaKey - Meta 键是否按下
 */
function parseSingleKeyCombination(keyString) {
  ...
  for (const part of parts) {
    if (MODIFIER_KEYS[part]) {
      ...
      console.error(`无效的按键组合: ${keyString}`);
      return null;
    }
  }
  ...
  if (!mainKeyPart) {
    if (Object.keys(MODIFIER_KEYS).includes(parts[parts.length - 1]) && parts.length === 1) {
      mainKeyPart = parts[parts.length - 1];
  ...
  if (mainKeyPart.length === 1) {
    ...
    return {
      key: keyChar,
      code: `Key${mainKeyPart.toUpperCase()}`,
      keyCode: charCode,
      charCode: charCode,
      modifiers,
    };
  ...
  console.error(`未知按键: ${mainKeyPart}`);
  return null;
}

/**
 * 分发键盘事件
 * @param {Object} parsedKeyInfo - 解析后的按键信息对象
 * @param {string} parsedKeyInfo.key - 按键名称
 * @param {string} parsedKeyInfo.code - 按键代码
 * @param {number} parsedKeyInfo.keyCode - 按键码值
 * @param {number} [parsedKeyInfo.charCode] - 字符码值
 * @param {Object} parsedKeyInfo.modifiers - 修饰键状态对象
 * @param {HTMLElement} element - 目标元素，将在该元素上触发键盘事件
 * @returns {Object} 返回事件分发的结果对象
 * @returns {boolean} returns.success - 事件是否成功分发
 * @returns {string} [returns.error] - 错误信息（如果分发失败）
 */
function dispatchKeyEvents(parsedKeyInfo, element) {
  if (!parsedKeyInfo) return { success: false, error: '无效的按键信息' };
  ...
  try {
    ...
    console.error(`分发按键事件失败 "${key}":`, error);
    return {
      success: false,
      error: `分发按键事件失败 "${key}": ${error.message}`,
    };
  }
}
 ...
 * 
 * 修改原因：
 * 1. 原参数 targetElement 接收元素对象，但从 CDP 层传递过来的是字符串选择器
 * 2. 需要在此函数内部将字符串选择器转换为元素对象
 * 3. 添加更好的错误处理，当元素不存在时返回明确的错误信息
 * 4. 支持两种输入格式：逗号分隔的按键序列（"H,e,l,l,o"）或普通字符串（"Hello"）
 * 
 * @param {string} keysSequenceString - 按键序列字符串，如 "H,e,l,l,o" 或 "Ctrl+A" 或 "Hello"
 * @param {string|HTMLElement|null} [targetSelector=null] - 目标元素选择器或元素对象，如果为 null 则使用当前活动元素
 * @param {number} [delay=0] - 按键间隔延迟（毫秒），0 表示无延迟
 * @returns {Promise<Object>} 返回键盘输入操作的结果对象
 * @returns {boolean} returns.success - 所有按键操作是否成功
 * @returns {string} returns.message - 操作结果消息
 * @returns {string} [returns.error] - 错误信息（如果操作失败）
 * @returns {Array<Object>} returns.results - 每个按键操作的详细结果数组
 * @returns {string} returns.results[].keyCombination - 按键组合字符串
 * @returns {boolean} returns.results[].success - 该按键操作是否成功
 * @returns {string} [returns.results[].error] - 该按键操作的错误信息
 * @returns {Object} [returns.targetElement] - 目标元素信息
 * @returns {string} returns.targetElement.tagName - 元素标签名
 * @returns {string} returns.targetElement.id - 元素 ID
 * @returns {string} returns.targetElement.className - 元素类名
 * @returns {string} [returns.targetElement.type] - 输入框类型
 */
async function simulateKeyboard(keysSequenceString, targetSelector = null, delay = 0) {
  try {
    // 获取目标元素
    // 修改说明：支持字符串选择器和元素对象两种方式
    let element;
    if (targetSelector) {
      if (typeof targetSelector === 'string') {
        ...
        if (!element) {
          return {
            success: false,
            error: `未找到元素: ${targetSelector}`,
            results: []
          };
        }
      } else {
        element = targetSelector; // 直接传递的元素对象
      }
    } else {
    ...
    // 智能解析按键序列
    // 如果包含逗号或特殊键（如 Ctrl+A），按逗号分割
    // 否则，按字符分割（用户输入普通文本）
    let keyCombinations;
    if (keysSequenceString.includes(',') || /\+/.test(keysSequenceString)) {
      // 逗号分隔格式：H,e,l,l,o 或 Ctrl+A,B,C
      keyCombinations = keysSequenceString
        .split(',')
        .map((k) => k.trim())
        .filter((k) => k.length > 0);
    } else {
      // 普通字符串格式：Hello -> ['H', 'e', 'l', 'l', 'o']
      keyCombinations = keysSequenceString.split('');
    }
    ...
    for (let i = 0; i < keyCombinations.length; i++) {
      ...
      if (dispatchResult.error) {
        console.warn(`模拟按键失败 "${comboString}": ${dispatchResult.error}`);
      }
 ...
 * @returns {boolean} 如果元素可交互返回 true，否则返回 false（被禁用或隐藏的元素不可交互）
 */
...
/**
 * 生成 CSS 选择器
 * @param {HTMLElement} el - 要生成选择器的 DOM 元素
 * @returns {string} 返回生成的 CSS 选择器字符串，优先使用 ID、data-testid、data-cy、name 属性，否则生成路径选择器
 */
...
/**
 * 获取可访问名称
 * @param {HTMLElement} el - 要获取可访问名称的 DOM 元素
 * @returns {string} 返回元素的可访问名称，按优先级：aria-labelledby > aria-label > label[for] > 父级 label > placeholder > value > textContent > title
 */
 ...
 * @returns {boolean} 如果文本中包含查询字符串的所有字符（按顺序）返回 true，否则返回 false（不区分大小写）
 */
...
/**
 * 创建元素信息对象
 * @param {HTMLElement} el - 要创建信息对象的 DOM 元素
 * @param {string} type - 元素类型（如 "button"、"link"、"input" 等）
 * @param {boolean} includeCoordinates - 是否包含坐标信息
 * @param {boolean|null} [isInteractiveOverride=null] - 是否可交互的覆盖值，null 表示自动检测
 * @returns {Object} 返回元素信息对象
 * @returns {string} returns.type - 元素类型
 * @returns {string} returns.selector - 生成的 CSS 选择器
 * @returns {string} returns.text - 元素的可访问名称或文本内容
 * @returns {boolean} returns.isInteractive - 元素是否可交互
 * @returns {boolean} returns.disabled - 元素是否被禁用
 * @returns {Object} [returns.coordinates] - 坐标信息（如果 includeCoordinates 为 true）
 * @returns {number} returns.coordinates.x - 元素中心点的 X 坐标
 * @returns {number} returns.coordinates.y - 元素中心点的 Y 坐标
 * @returns {Object} returns.coordinates.rect - 元素的位置和尺寸信息
 */
 ...
 * @param {string} [options.textQuery] - 文本查询字符串，用于模糊匹配元素的可访问名称
 * @param {boolean} [options.includeCoordinates=true] - 是否在结果中包含坐标信息
 * @param {Array<string>} [options.types] - 要查找的元素类型数组（如 ["button", "link", "input"]），默认为所有类型
 * @param {boolean} [options.strictVisibility=true] - 是否使用严格的可见性检查（包括遮挡检测）
 * @returns {Array<Object>} 返回找到的交互元素信息对象数组
 */
function findInteractiveElements(options = {}) {
  const { textQuery, includeCoordinates = true, types = Object.keys(ELEMENT_CONFIG), strictVisibility = true } = options;

  // 根据 strictVisibility 选择可见性检查函数
  const visibilityCheck = strictVisibility ? isElementVisible : isElementBasicVisible;
  ...
  for (const el of uniqueElements) {
    if (!visibilityCheck(el) || !isElementInteractive(el)) continue;
 ...
 * 通过文本查找元素（多层回退策略）
 * @param {Object} [options={}] - 查找选项
 * @param {string} [options.textQuery] - 文本查询字符串，用于匹配元素文本
 * @param {boolean} [options.includeCoordinates=true] - 是否在结果中包含坐标信息
 * @param {boolean} [options.strictVisibility=true] - 是否使用严格的可见性检查（包括遮挡检测）
 * @returns {Array<Object>} 返回找到的元素信息对象数组
 * @description 查找策略：
 * 1. 第一层：查找匹配文本的交互元素
 * 2. 第二层：找到文本节点，然后找其交互祖先元素
 * 3. 第三层：返回任何包含文本的可见元素
 */
function findElementsByTextWithFallback(options = {}) {
  const { textQuery, includeCoordinates = true, strictVisibility = true } = options;

  // 根据 strictVisibility 选择可见性检查函数
  const visibilityCheck = strictVisibility ? isElementVisible : isElementBasicVisible;

  if (!textQuery) {
    return findInteractiveElements({ ...options, types: Object.keys(ELEMENT_CONFIG), strictVisibility });
  }

  // 第一层：查找匹配文本的交互元素
  let results = findInteractiveElements({ ...options, types: Object.keys(ELEMENT_CONFIG), strictVisibility });
  if (results.length > 0) {
    return results;
  }
  ...
  if (textNodes.snapshotLength > 0) {
    for (let i = 0; i < textNodes.snapshotLength; i++) {
      ...
      if (parentElement) {
        ...
        if (
          interactiveAncestor &&
          visibilityCheck(interactiveAncestor) &&
  ...
  for (let i = 0; i < textNodes.snapshotLength; i++) {
    const parentElement = textNodes.snapshotItem(i).parentElement;
    if (parentElement && visibilityCheck(parentElement)) {
 ...
 * 
 * 修改原因：
 * 1. 修复逻辑，支持按 types 查询交互元素
 * 2. 优先级：selector > textQuery > types
 * 3. 如果什么都不传，返回所有交互元素
 * 
 * @param {Object} [options={}] - 查询选项
 * @param {string} [options.selector] - CSS 选择器，用于查找匹配的元素
 * @param {string} [options.textQuery] - 文本查询字符串，用于模糊匹配元素文本
 * @param {Array<string>} [options.types] - 元素类型数组（如 ["button", "link"]）
 * @param {boolean} [options.includeCoordinates=true] - 是否在结果中包含坐标信息
 * @param {boolean} [options.strictVisibility=true] - 是否使用严格的可见性检查
 * @returns {Object} 返回查询结果对象
 * @returns {boolean} returns.success - 查询是否成功
 * @returns {Array<Object>} returns.elements - 找到的元素信息对象数组
 * @returns {string} [returns.error] - 错误信息（如果查询失败）
 */
function getInteractiveElements(options = {}) {
  try {
    ...
    if (options.selector) {
      // 使用自定义选择器
      const foundEls = Array.from(document.querySelectorAll(options.selector));
      elements = foundEls.map((el) =>
        createElementInfo(
          el,
          'selected',
          options.includeCoordinates !== false,
          isElementInteractive(el)
        )
      );
    } else if (options.textQuery) {
      // 按文本查询
      elements = findElementsByTextWithFallback(options);
    } else {
      // 按类型查询或查询所有交互元素
      elements = findInteractiveElements(options);
    }
    return {
      success: true,
      elements: elements,
...
/**
 * 获取固定/粘性定位元素
 * @returns {Array<Object>} 返回所有固定或粘性定位的元素信息数组
 * @returns {HTMLElement} returns[].element - 元素对象
 * @returns {string} returns[].originalDisplay - 元素原始的 display 样式
 * @returns {string} returns[].originalVisibility - 元素原始的 visibility 样式
 */
 ...
 * 隐藏固定元素
 * @returns {void}
 * @description 隐藏页面中所有 position 为 fixed 或 sticky 的元素，用于截图时避免遮挡
 */
 ...
 * 显示固定元素
 * @returns {void}
 * @description 恢复之前隐藏的固定元素，将它们的 display 样式恢复为原始值
 */
 ...
 * @param {boolean} [options.fullPage=false] - 是否进行全页面截图（如果为 true 会隐藏固定元素）
 * @returns {Promise<Object>} 返回准备操作的结果对象
 * @returns {boolean} returns.success - 操作是否成功
 * @description 准备页面用于截图，会隐藏页面的滚动条，如果需要全页面截图还会隐藏固定元素
 */
async function preparePageForCapture(options = {}) {
...
/**
 * 获取页面详情
 * @returns {Object} 返回页面尺寸和滚动信息对象
 * @returns {number} returns.totalWidth - 页面总宽度（包括滚动区域）
 * @returns {number} returns.totalHeight - 页面总高度（包括滚动区域）
 * @returns {number} returns.viewportWidth - 视口宽度
 * @returns {number} returns.viewportHeight - 视口高度
 * @returns {number} returns.devicePixelRatio - 设备像素比
 * @returns {number} returns.currentScrollX - 当前水平滚动位置
 * @returns {number} returns.currentScrollY - 当前垂直滚动位置
 */
...
/**
 * 获取元素详情
 * @param {string} selector - CSS 选择器，用于定位元素
 * @returns {Promise<Object>} 返回元素详情对象
 * @returns {Object} returns.rect - 元素位置和尺寸信息
 * @returns {number} returns.rect.x - 元素 X 坐标（相对于视口）
 * @returns {number} returns.rect.y - 元素 Y 坐标（相对于视口）
 * @returns {number} returns.rect.width - 元素宽度
 * @returns {number} returns.rect.height - 元素高度
 * @returns {number} returns.devicePixelRatio - 设备像素比
 * @throws {Error} 如果未找到指定元素会抛出错误
 */
async function getElementDetails(selector) {
  const element = document.querySelector(selector);
  if (!element) {
...
/**
 * 滚动页面
 * @param {number} x - 目标水平滚动位置
 * @param {number} y - 目标垂直滚动位置
 * @param {number} [scrollDelay=300] - 滚动完成后的等待时间（毫秒）
 * @returns {Promise<Object>} 返回滚动操作的结果对象
 * @returns {boolean} returns.success - 操作是否成功
 * @returns {number} returns.newScrollX - 滚动后的水平位置
 * @returns {number} returns.newScrollY - 滚动后的垂直位置
 */
async function scrollPage(x, y, scrollDelay = 300) {
...
/**
 * 重置页面
 * @param {number} [scrollX] - 要恢复的水平滚动位置
 * @param {number} [scrollY] - 要恢复的垂直滚动位置
 * @returns {Object} 返回重置操作的结果对象
 * @returns {boolean} returns.success - 操作是否成功
 * @description 重置页面状态，恢复滚动条显示、显示固定元素，并可选择性地恢复滚动位置
 */
function resetPageAfterCapture(scrollX, scrollY) {
 ...
 * @param {string} method - HTTP 请求方法（如 "GET"、"POST"、"PUT"、"DELETE" 等）
 * @param {Object} [headers] - 请求头对象
 * @param {string|Object} [body] - 请求体（GET 和 HEAD 请求会被忽略）
...
async function sendNetworkRequest(url, method, headers, body, timeout = 30000) {
  try {
    ...
    try {
      ...
      responseData.body = `[解析响应体失败: ${error.message}]`;
    }
    ...
    console.error('发送网络请求失败:', error);
    return {
      success: false,
      error: `发送网络请求失败: ${error.message}`,
    };
  }
}
...
/**
 * Readability 内容提取引擎
 * 基于 Mozilla Readability.js
 * Copyright (c) 2010 Arc90 Inc (Apache License 2.0)
 * 
 * @constructor
 * @param {Document} doc - DOM 文档对象，需要包含完整的 HTML 结构
 * @param {Object} [options] - 配置选项对象
 * @param {boolean} [options.debug=false] - 是否启用调试模式
 * @param {number} [options.maxElemsToParse=0] - 最大解析元素数量，0 表示无限制
 * @param {number} [options.nbTopCandidates=5] - 考虑的前 N 个候选元素数量
 * @param {number} [options.charThreshold=500] - 文章的最小字符数阈值
 * @param {Array<string>} [options.classesToPreserve] - 需要保留的 CSS 类名数组
 * @param {boolean} [options.keepClasses=false] - 是否保留元素的 CSS 类名
 * @param {Function} [options.serializer] - 自定义序列化函数，默认为返回 innerHTML
 * @param {boolean} [options.disableJSONLD=false] - 是否禁用 JSON-LD 元数据提取
 * @param {RegExp} [options.allowedVideoRegex] - 允许的视频 URL 正则表达式
 * @param {number} [options.linkDensityModifier=0] - 链接密度修正值
 * @throws {Error} 如果 doc 参数不是有效的文档对象会抛出错误
 */
...
Readability.prototype = {
   ...
   * 工作流程:
   *  1. 准备文档：移除脚本标签、CSS 等
   *  2. 构建 Readability 的 DOM 树
   *  3. 从当前 DOM 树中提取文章内容
   *  4. 替换当前 DOM 树为新的树
   *  5. 返回提取的结果
   *
   * @returns {Object|null} 返回解析后的文章对象，如果解析失败返回 null
   * @returns {string} returns.title - 文章标题
   * @returns {string} [returns.byline] - 文章作者署名
   * @returns {string} [returns.dir] - 文章文本方向（"ltr" 或 "rtl"）
   * @returns {string} [returns.lang] - 文章语言代码
   * @returns {string} returns.content - 文章的 HTML 内容
   * @returns {string} returns.textContent - 文章的纯文本内容
   * @returns {number} returns.length - 文章文本内容的长度（字符数）
   * @returns {string} [returns.excerpt] - 文章摘要
   * @returns {string} [returns.siteName] - 网站名称
   * @returns {string} [returns.publishedTime] - 发布时间
   * @throws {Error} 如果文档元素数量超过 maxElemsToParse 限制会抛出错误
   **/
  parse() {
...
// 修改原因：在 Electron 环境中，chrome 对象不存在，需要条件检查
// 这段代码仅在 Chrome 扩展环境中执行，Electron 环境会跳过
...
/**
 * 从页面提取元数据
 * @returns {Object} 返回页面元数据对象
 * @returns {string} returns.title - 页面标题
 * @returns {string} returns.description - 页面描述
 * @returns {string} returns.author - 作者信息
 * @returns {string} returns.keywords - 关键词
 * @returns {string} returns.published - 发布日期
 * @returns {string} returns.siteName - 网站名称
 */
...
/**
 * 从 iframe 元素提取内容
 * @returns {string} 返回所有同源 iframe 的文本内容组合（用换行符分隔）
 * @description 只提取同源且可见的 iframe 内容，跨域 iframe 会被忽略
 */
 ...
 * @returns {boolean} 如果 iframe 同源返回 true，否则返回 false
 * @description 通过尝试访问 iframe 的 contentDocument 来判断是否同源
 */
 ...
 * @returns {string} 返回清理后的文本，会压缩多余空白字符并限制最大长度
 * @description 清理多余的空白字符，将多个连续空格和换行符压缩，并截断到最大长度限制
 */
...
/**
 * 清理 HTML 内容，移除样式标签和内联样式
 * @param {string} html - 要清理的 HTML 内容字符串
 * @returns {string} 返回清理后的 HTML 字符串
 * @description 移除所有 style 标签、link 标签、script 标签，移除所有内联 style 属性，将 SVG 替换为占位符，移除纯数据属性的空元素和 HTML 注释
 */
...
/**
 * 获取网页文本内容（使用 Readability）
 * @param {string|null} [selector=null] - 可选的 CSS 选择器，如果指定则只提取该元素的文本内容
 * @returns {Object} 返回文本提取结果对象
 * @returns {boolean} returns.success - 提取是否成功
 * @returns {string} [returns.textContent] - 提取的文本内容
 * @returns {string} [returns.selector] - 使用的选择器（如果指定了 selector）
 * @returns {Object} [returns.article] - 文章信息对象（如果使用 Readability 提取）
 * @returns {string} returns.article.title - 文章标题
 * @returns {string} [returns.article.byline] - 文章作者
 * @returns {string} [returns.article.siteName] - 网站名称
 * @returns {string} [returns.article.excerpt] - 文章摘要
 * @returns {string} [returns.article.lang] - 文章语言
 * @returns {string} [returns.article.content] - 文章 HTML 内容
 * @returns {Object} [returns.metadata] - 页面元数据对象
 * @returns {boolean} [returns.fallback] - 是否为降级方案（Readability 提取失败时使用）
 * @returns {string} [returns.error] - 错误信息（如果提取失败）
 */
function getTextContent(selector = null) {
  try {
    ...
    if (article && article.textContent) {
      ...
      if (iframeContent && iframeContent.trim().length > 20) {
        fullContent += '\n\n--- 嵌入内容 ---\n\n' + iframeContent;
      }
...
/**
 * 获取网页 HTML 内容
 * @param {string|null} [selector=null] - 可选的 CSS 选择器，如果指定则只提取该元素的 HTML 内容
 * @returns {Object} 返回 HTML 提取结果对象
 * @returns {boolean} returns.success - 提取是否成功
 * @returns {string} [returns.htmlContent] - 清理后的 HTML 内容
 * @returns {string} [returns.selector] - 使用的选择器（如果指定了 selector）
 * @returns {string} [returns.error] - 错误信息（如果提取失败）
 * @description HTML 内容会经过清理，移除样式、脚本等无关内容
 */
function getHtmlContent(selector = null) {
  try {
    ...
    return {
      success: false,
      error: `获取 HTML 内容失败: ${error.message}`,
    };
  }
}
 ...
 * 提供浏览器自动化所需的各种功能函数
 * 
 * @namespace __browserHelpers
 * @property {Function} clickElement - 点击元素函数
 * @property {Function} simulateClick - 模拟鼠标点击函数
 * @property {Function} fillElement - 填充表单元素函数
 * @property {Function} simulateKeyboard - 模拟键盘输入函数
 * @property {Function} getInteractiveElements - 获取交互元素函数
 * @property {Function} findInteractiveElements - 查找交互元素函数
 * @property {Function} findElementsByTextWithFallback - 通过文本查找元素函数（多层回退）
 * @property {Function} preparePageForCapture - 准备页面截图函数
 * @property {Function} getPageDetails - 获取页面详情函数
 * @property {Function} getElementDetails - 获取元素详情函数
 * @property {Function} scrollPage - 滚动页面函数
 * @property {Function} resetPageAfterCapture - 重置页面函数
 * @property {Function} sendNetworkRequest - 发送网络请求函数
 * @property {Function} getTextContent - 获取网页文本内容函数
 * @property {Function} getHtmlContent - 获取网页 HTML 内容函数
 * @property {Function} extractPageMetadata - 提取页面元数据函数
 * @property {Function} extractIframeContent - 提取 iframe 内容函数
 * @property {Function} cleanHtmlContent - 清理 HTML 内容函数
 * @property {Function} cleanContent - 清理文本内容函数
 * @property {Function} isElementVisible - 检查元素是否可见函数（严格检查）
 * @property {Function} getElementInfo - 获取元素信息函数
 * @property {Function} isElementInteractive - 检查元素是否可交互函数
 * @property {Function} generateSelector - 生成 CSS 选择器函数
 * @property {Function} getAccessibleName - 获取可访问名称函数
 */
window.__browserHelpers = {
  // 点击功能
  clickElement,
  simulateClick,
