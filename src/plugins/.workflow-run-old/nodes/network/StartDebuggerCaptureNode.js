...
/**
 * HTTP 启动 Debugger 网络捕获节点
 * 通过 HTTP 调用 network:startDebuggerCapture channel
 */
export class NetworkStartDebuggerCaptureNode extends BaseNode {
  type = "http:network:startDebuggerCapture";
  label = "启动Debugger捕获";
  description = "通过 HTTP API 启动 Debugger 网络捕获（含响应体）";
  category = "网络操作";
  ...
  async execute(config, inputs, workflowContext) {
    ...
    context.logger.debug("启动Debugger捕获", { viewId, options });

    const response = await context.http.invoke("network:startDebuggerCapture", viewId, options);
